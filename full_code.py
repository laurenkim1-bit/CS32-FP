...
def input_one_task() -> dict:
    ...
    due_date        = _prompt_date("\n  Due date (YYYY-MM-DD) or Enter for today: ")
    estimated_hours = _prompt_float("\n  Estimated hours to complete (e.g. 1.5): ", 0.1, 24.0)
...
def collect_tasks(load_existing: bool = True) -> list:
    ...
    while True:
        ...
        action = _choose_from_list(
            ...
            ["Add a new task", "Edit task status", "View current tasks", "Done adding tasks"],
        )
        ...
        elif action == "Edit task status":
            if not tasks:
                print("  No tasks yet.")
                continue
            print_tasks(tasks)
            idx = _prompt_int("  Enter task number to update: ", 1, len(tasks)) - 1
            sorted_tasks = sort_tasks_by_priority(tasks)
            t = sorted_tasks[idx]
            t["status"] = _choose_from_list("  New status:", ["Not started", "In progress", "Done"])
            if t["status"] == "Done":
                t["hours_remaining"] = 0
            _save_tasks(tasks)
            print(f"  ✅  Updated '{t['name']}' → {t['status']}")
...
def compute_priority(task: dict, today: datetime.date = None) -> float:
    ...
    importance_score = (task["importance"] - 1) / 4.0
    difficulty_score = (task["difficulty"] - 1) / 4.0
    ...
    for task in sorted_tasks:
        ...
        for strict in (True, False):
            ...
            for slot in slots:
                ...
                # Hard tasks → productive hours (strict pass only)
                is_hard      = task["difficulty"] >= 4
