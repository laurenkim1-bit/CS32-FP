...
Smart To-Do List Scheduler
  ...
  3. Input tasks (name, category, due date, estimated time, difficulty, importance)
  4. Collect user preferences (productive hours, light days, category prefs)
...
Changes:
Task input is now just 4 questions:
1. Task name
2. Days until due (type 3 instead of 2025-05-10)
3. Importance: h / m / l instead of a 1–5 rating
4. Estimated hours

Removed entirely: difficulty, status, category — all cut from input. Category defaults to "General" internally (still stored so nothing breaks), difficulty defaults to a neutral value, status defaults to "Not started".
Priority formula updated to match — now 50% deadline, 40% importance, 10% quick-win bias. Difficulty is no longer a factor.
"""
...
def input_one_task() -> dict:
    ...
    task = {
        "name":            name,
        "category":        "General",   # default; no longer asked
        "due_date":        due_date,
        "estimated_hours": estimated_hours,
        "difficulty":      2,           # neutral default; no longer asked
...
# ---------------------------------------------------------------------------
# Scheduler – wires preferences + multi-session splitting
# ---------------------------------------------------------------------------
...
def _time_matches(slot_hour: int, preferred: str) -> bool:
    if preferred == "any" or not preferred:
        return True
    return _time_of_day_label(slot_hour) == preferred
...
def _explain(task: dict, slot: dict, prefs: dict) -> str:
    ...
    cat_pref = prefs["category_time_prefs"].get(task["category"], "")
    if cat_pref and _time_matches(slot["start"].hour, cat_pref):
        reasons.append(f"matches your {cat_pref} preference for {task['category']}")
...
def schedule_tasks(tasks: list, free_slots: list, prefs: dict) -> list:
    ...
    for task in sorted_tasks:
        ...
        # Two passes: first try preferred slots, then any slot
        for strict in (True, False):
            ...
            for slot in slots:
                ...
                cat_pref = prefs["category_time_prefs"].get(task["category"], "any")
                if strict and not _time_matches(slot_hour, cat_pref):
                    continue
                ...
                if (strict
                        and prefs["hard_tasks_in_productive"].startswith("Yes")
                        and is_important
...
def print_week_view(events: list, schedule: list, free_slots: list) -> None:
    """
    Print a unified day-by-day view showing calendar events,
    scheduled tasks, and remaining free time together.
    """
    tz = ZoneInfo(TIMEZONE)

    # Build a combined list of items per day
    ...
    for s in schedule:
        total_sessions = s["task"]["session_count"] - 1  # session_count was incremented after scheduling
    ...
    # Sort days
    today     = datetime.datetime.now(tz).date()
    all_days  = sorted(day_items.keys())
